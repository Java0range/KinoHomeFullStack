...
const FilmsWindow = ({ filmGenres }: Props) => {
    const router = useRouter();
    const [isCategory, setIsCategory] = useState(false);
    const [filmsFiltersGenres, setFilmsFiltersGenres] = useState<string[]>([]);
    const addGenre = (genre: string) => {
        setFilmsFiltersGenres(prev =>
            prev.includes(genre)
    ...
    const moviesRequest = useMovies("film", filmsFiltersGenres, getMoviesSize());
    ...
    return (
        <div className="mt-3 w-full flex flex-col">
            <div className="select-none flex gap-4 w-full items-center max-md:justify-center">
                <h3 className="text-red-600 text-3xl font-bold">Фильмы</h3>
                <div
                ...
                {filmGenres.map(genre => (
                    <div
                        ...
                        className={`select-none p-1.5 rounded-xl text-white bg-red-600 flex justify-center cursor-pointer hover:shadow-[0_0_10px_rgb(255,255,255)] shadow-red-600 transition-all duration-500 ${
                            filmsFiltersGenres.includes(genre) && "-translate-y-2"
                        }`}
                    >
                        <p>{genre}</p>
                    </div>
